#!/usr/bin/env python3
"""
Fallback metadata sender using only standard library (no requests dependency)
Usage: python3 send_metadata_fallback.py <cluster_name> <django_url>
"""
import json
import sys
import os
...
def send_metadata_to_django(cluster_name, django_url):
    """Send all metadata to Django using urllib (no requests dependency)"""
    
    print(f"=== DEBUGGING CLUSTER METADATA (FALLBACK) ===")
    ...
    try:
        ...
        print(f"Master IP detected: {master_ip}")
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                'User-Agent': 'Jenkins-Metadata-Fallback/1.0'
            }
        )
            ...
            if response.status == 200:
                try:
                    ...
                    if 'master_ip' in result:
                        print(f"Master IP confirmed: {result['master_ip']}")
                    return True
                except json.JSONDecodeError:
                    print(f"✅ Success (non-JSON response): {response_data[:200]}...")
                    return True
            else:
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: python3 send_metadata_fallback.py <cluster_name> <django_url>")
        print("Example: python3 send_metadata_fallback.py k8s-cluster-123 https://labngoprek.my.id")
        sys.exit(1)
