#!/bin/bash
# ======================================================================
# Debian 12 Kubernetes-Ready Template Creation Script
# ======================================================================
# This script creates a pre-configured Debian 12 template with all
# Kubernetes prerequisites installed and configured.
#
# Usage: Run this script INSIDE a fresh Debian 12 VM that will become
...
log "🚀 Starting Debian 12 Kubernetes-Ready Template Creation"
...
echo "deb [signed-by=/usr/share/keyrings/kubernetes-archive-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v${K8S_VERSION}/deb/ /" | tee /etc/apt/sources.list.d/kubernetes.list
...
log "Holding Kubernetes packages at current versions..."
...
# Disable UFW if present (Kubernetes manages its own firewall rules)
...
if apt-mark showhold | grep -q kubelet; then
    info "✓ Kubernetes packages are held"
else
    warning "✗ Kubernetes packages not held"
fi
...
# COMPLETION
# ======================================================================
log "🎉 Debian 12 Kubernetes-Ready Template Creation Complete!"
echo
info "📋 TEMPLATE SUMMARY:"
info "   • Operating System: Debian 12 ($(lsb_release -cs))"
...
warning "   4. Update Ansible to use template: t-debian12-k8s-ready"
echo
