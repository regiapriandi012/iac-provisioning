...
# Output untuk script automation
output "automation_script" {
  description = "Bash script to run ansible automatically"
  value = <<-EOT
#!/bin/bash
set -e

echo "🚀 Starting Ansible automation for VMs..."

# Create inventory file
cat > inventory.ini << 'EOF'
${chomp(local.ansible_inventory_content)}
EOF

# Create playbook file
cat > nginx-playbook.yml << 'EOF'
${chomp(local.ansible_playbook_content)}
EOF

echo "📝 Files created:"
echo "  - inventory.ini"
echo "  - nginx-playbook.yml"
echo ""
...
ansible all -i inventory.ini -m ping --timeout=10

if [ $? -eq 0 ]; then
    echo "✅ All hosts are reachable!"
    echo ""
    echo "🚀 Running nginx installation playbook..."
...
%{for k, v in local.vm_data~}
    echo "  - ${v.vm_name_final}: http://${replace(v.ip_address, "/24", "")}"
%{endfor~}
else
    echo "❌ Some hosts are not reachable. Check SSH connectivity."
    echo "💡 Try running: ssh root@<IP_ADDRESS> to test manual connection"
fi
EOT
}

# Local values untuk reuse
locals {
...
%{for k, v in local.vm_data~}
%{if can(regex("master", lower(v.vm_name_original)))~}
${v.vm_name_final} ansible_host=${replace(v.ip_address, "/24", "")} vmid=${v.vmid}
%{endif~}
%{endfor~}
